...
import blockchain
from cryptography import verify
...
class UTXOPool:
    ...
    def handleCoinbase(self, coinbaseTx):
        ...
        if sum([txOut.value for txOut in coinbaseTx.txOuts]) > blockchain.COINBASE_AMT:
            raise ("Coinbase tx can only create {} coins".format(
                blockchain.COINBASE_AMT))
    ...
    def verifyTx(self, tx):
        ...
        for idx, (txIn, prevTxOut) in enumerate(zip(tx.txIns, prevTxOuts)):
            ...
            try:
                ...
                verify(tx.getDataToSign(idx), signature, prevTxOut.address)
            except:
        ...
        if created > consumed:
            raise ValueError(
                "Sum of outputs is greater than previous outputs!")
